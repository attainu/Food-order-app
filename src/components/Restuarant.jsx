        ...
        return (
            <div>
               <form onSubmit={this.handleSubmit}>
                   <input type="text" name="place" value={this.state.place} onChange={this.handleChange} id=""/>
                   <input type="submit" value="search"/>
               </form>
              <div className="res2"> {this.props.hotel!==null?<>{this.props.hotel.restaurants.map(res=><Restuarantlist key={uuidv4()} restuarant={res}/>)}</>:""}</div>
            </div>
        )
    }
}
